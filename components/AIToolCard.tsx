...
const AIToolCard: React.FC<AIToolCardProps> = ({
    ...
    return (
        <article
            key={id}
            className={`relative h-full isolate flex flex-col group ${
                variant === 'compact' ? 'rounded-xl shadow-md hover:shadow-lg' : 'rounded-2xl shadow-lg hover:shadow-xl'
            } border border-gray-200 bg-white transition-shadow ${className}`}
        >
            ...
            <div className={`relative z-20 ${paddingClass} border-b border-gray-100 pointer-events-none`}>
                <div className={`flex items-start ${variant === 'compact' ? 'gap-3 mb-2' : 'gap-4 mb-4'}`}>
                    <div
                        ...
                        } rounded-xl overflow-hidden bg-gray-200 flex-shrink-0`}
                    >
                        {logoUrl ? (
                    ...
                    <div className="flex-1">
                        ...
                        <div
                            className={`flex flex-nowrap overflow-hidden ${
                                variant === 'compact' ? 'gap-1 mt-0.5' : 'gap-1.5 mt-1'
                            ...
                            {showTags.map((badge) => (
                                <span
                                    ...
                                    className={`${getBadgeClass(
                                        ...
                                        variant === 'compact' ? 'h-5 px-2 text-[10px]' : 'h-6 px-2.5 text-xs'
                ...
                {showVersionInfo && (
                    <div
                        ...
                        } text-gray-400 bg-white/40 backdrop-blur-sm pr-2 py-1 rounded-md pointer-events-none whitespace-nowrap`}
                    >
                        {latestVersion && relativeTime ? (
                            <div className="font-medium">
                                {latestVersion} Released {relativeTime}
                            </div>
                        ) : latestVersion ? (
            ...
            {(hasKeyFindings || showWhoSection) && (
                <div
                    className={`relative z-20 ${
                        variant === 'compact' ? 'px-3 pt-2 pb-2' : 'px-6 pt-4 pb-4'
                    ...
                    <div className={`grid grid-cols-1 sm:grid-cols-2 ${sectionGap}`}>
                        ...
                        <div>
                            <h4
                                className={`${
                                    variant === 'compact' ? 'text-xs' : 'text-sm'
                                } font-semibold text-gray-900 ${variant === 'compact' ? 'mb-1' : 'mb-2'}`}
                            >
                                Key Features
                            </h4>
                            {hasKeyFindings ? (
                                <ul className={variant === 'compact' ? 'space-y-0.5' : 'space-y-1'}>
                                    {keyFindings!.map((k, i) => (
                                        <li
                                            key={i}
                                            className={`flex items-start gap-1.5 ${keyFindingFont} text-gray-700`}
                                        >
                                            <Check
                                                className={`${iconSizeClass} mt-0.5 text-green-600 flex-shrink-0`}
                                                aria-hidden="true"
                                            />
                                            <span>{k}</span>
                                        </li>
                                ...
                                <p className={`${variant === 'compact' ? 'text-xs' : 'text-sm'} text-gray-400 italic`}>
                                    No key features yet.
                                </p>
                            )}
                        </div>
                                ...
                                <div className={`flex flex-col ${variant === 'compact' ? 'gap-0.5' : 'gap-1.5'}`}>
                                    {hasWhoIsItFor
                                        ? whoIsItForList!.map((item, i) => (
                                              ...
                                              >
                                                  <User
                                                      className={`${userIconClass} text-blue-400 flex-shrink-0`}
                                                      aria-hidden="true"
                                                  />
                                                  <span>{item}</span>
                                              </div>
            ...
            <div
                className={`relative z-20 ${
                    variant === 'compact' ? 'px-3 pb-3 pt-2' : 'px-6 pb-6 pt-4'
            ...
            >
                <div className="flex items-center justify-between">
                    ...
                    <Link
                        href={ctaHref}
                        className={`pointer-events-auto relative z-[70] inline-flex ${
                            variant === 'compact' ? 'text-xs' : 'text-sm'
                        } font-medium text-gray-600 underline underline-offset-4 hover:text-gray-900 transition-colors ml-auto`}
                    >
                        Full Review
                    </Link>
                </div>
