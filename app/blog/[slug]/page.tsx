...
export default async function BlogDetailPage({ params }: BlogPageProps) {
    ...
    const authorInitials = authorName
        ...
        .join('');
    const authorBioRaw = post.blog?.authorBio ?? post.author?.node?.description ?? null;
    ...
    const [relatedData, allTagRes, navTagsRes, navMenuRes, branding, topPicksRes, categoriesRes] = await Promise.all([
        tagSlugs.length > 0
    ...
    let recommendedArticles = relatedPosts
        .map((relatedPost: any) => {
            const relatedTagSlugs = relatedPost?.tags?.nodes?.map((t: any) => t.slug) ?? [];
            const overlap = relatedTagSlugs.filter((slug: string) => tagSlugs.includes(slug));
